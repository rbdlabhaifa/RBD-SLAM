...
int main() {
    ...
    try {
        std::vector<std::vector<double>> data_points = reader.read_file(file_path);
        // for (const auto& point : data_points) {
        ...
        //     std::cout << "New x: " << point[0] << ", y: " << point[1] << ", z: " << point[2] << std::endl;
        // }
        ...
        std::vector<double> drone_position = analyzer.find_middle_point(projected_data);
        std::vector<double> avg_distances = analyzer.calculate_average_distances(projected_data, drone_position);
        std::vector<size_t> nan_indices; 
        std::pair<std::vector<double>, std::vector<double>> exit_result = analyzer.find_exit(projected_data, drone_position, avg_distances);
