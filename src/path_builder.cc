...
PathBuilder::split_distances(const std::vector<double> &distances)
{
    ...
    pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud)
{
    ...
    std::cout << "min_val is " << min_val << " mean_val is " << mean_val
              << std::endl;
    ...
    const std::vector<std::unique_ptr<geos::geom::Geometry>> &polygons)
{
    ...
    path_to_the_unknown = {path_to_the_unknown.begin(),
                           path_to_the_unknown.end() -
                               parts[parts.size() - 1].size()};
