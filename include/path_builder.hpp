...
class PathBuilder
{
     ...
     */
    static std::size_t
    get_last_change(const std::vector<std::vector<std::size_t>> &polygon_idxs);
    ...
    split_distances(const std::vector<double> &distances);

    // NOTE: This function is not used at the moment, delete?
    static pcl::PointXYZ
    get_point_of_interest(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud);
