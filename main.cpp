...
bool is_valid_movement(pcl::PointCloud<pcl::PointXYZ>::ConstPtr cloud,
  ...
  float radius_search = 0.2 * ScaleFactor;
...
int main(int argc, char **argv) {
  ...
  pcl::PointXYZ knownPoint1 = {-0.264355, -0.105879 + 0.1, 0.166656};
  ...
  for (pcl::PointXYZ p : *navigate_data1) {
    ...
    viewer->addSphere(p, 0.15 * ScaleFactor, 0.9, 0.2, 0.0, ss.str());
    index++;
  }
  ...
  if (bfs.reached(get_random_node_from_RRT)) {
    ...
    while (prev != lemon::INVALID) {
      ...
      viewer->addSphere(nodeToPoint[prev], 0.15 * ScaleFactor, 0.1, 0.2, 0.9,
                        ss.str());
      index++;
    }
    std::cout << std::endl;
